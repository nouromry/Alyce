...
const OrdersTable: React.FC<OrdersTableProps> = ({ filteredOrders }) => {
  ...
  const handleViewOrder = (order: Order) => {
    if (order.status === "Livrée") {
      router.push(`/order-details-livree/${order.id}`);
    }
    if (order.status === "En cours") {
      router.push(`/order-details-encours/${order.id}`);
    }
  };
        ...
        <tbody>
          {filteredOrders.map((order) => (
            <tr key={order.id} className="border-b border-gray-300 text-center">
              ...
              <button
               className={`text-pink-500 ${order.status === "Livrée" || order.status === "En cours" ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`}
